setTimeout(()=> {
    jQuery(function ($) {
        // Development Slider Progressbar JS
        function progress() {
            var windowScrollTop = $(window).scrollTop();
            var docHeight = $('.horizontal-scroll-section').height() - 10;
            var windowHeight = $(window).height();
            var progress = (windowScrollTop / (docHeight - windowHeight)) * 100;
            var $bgColor = progress > 100 ? '' : '#EF4E31';
            $('.webdev-scrolls-secs .bar').width(progress + '%').css({ backgroundColor: $bgColor });
        }
        progress();
        ...
        function setScene($el) {
            ...
            if (bounding.top > winHeight) {
                ...
                if (bounding.top <= 0) {
                    // Development Slider Progressbar Function Call
                    progress();
